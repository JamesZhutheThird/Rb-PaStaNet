...
class ResNet50(): 
    def __init__(self):
        ...
        self.cls_rule_vec = tf.placeholder(tf.float32,shape=[None,512,10],name = 'cls_rule_vec')
    ...
    def vec_attention(self, cls_prob_vec,cls_rule_vec, fc5_P0, fc5_P1, fc5_P2, fc5_P3, fc5_P4, fc5_P5, fc5_O, initializer, is_training, name):
        with tf.variable_scope(name) as scope:
            print('cls_prob_vec:')
            print(cls_prob_vec.shape)
#            print(fc5_P0.shape)
            print(cls_prob_vec.get_shape())
            print(cls_rule_vec.get_shape())
#            tf.multiply(cls_prob_vec,np.zeros((20,111)))
            print('1:',fc5_P0.get_shape(), fc5_P1.get_shape(), fc5_P2.get_shape(), fc5_P3.get_shape(), fc5_P4.get_shape(), fc5_P5.get_shape())
            ...
            print('1:',fc5_P0.get_shape(), fc5_P1.get_shape(), fc5_P2.get_shape(), fc5_P3.get_shape(), fc5_P4.get_shape(), fc5_P5.get_shape())
            fc5_P0 = tf.multiply(fc5_P0, tf.reshape((cls_rule_vec[:,:,0:1]+cls_rule_vec[:,:,3:4])/2,[-1,512]))
            fc5_P1 = tf.multiply(fc5_P1, tf.reshape((cls_rule_vec[:,:,1:2]+cls_rule_vec[:,:,2:3])/2,[-1,512]))
            fc5_P2 = tf.multiply(fc5_P2, tf.reshape(cls_rule_vec[:,:,4:5],[-1,512]))
            fc5_P3 = tf.multiply(fc5_P3, tf.reshape((cls_rule_vec[:,:,6:7]+cls_rule_vec[:,:,9:10])/2,[-1,512]))
            fc5_P4 = tf.multiply(fc5_P4, tf.reshape((cls_rule_vec[:,:,7:8]+cls_rule_vec[:,:,8:9])/2,[-1,512]))
            fc5_P5 = tf.multiply(fc5_P5, tf.reshape(cls_rule_vec[:,:,5:6],[-1,512]))
...
#       return fc5_P0, fc5_P1, fc5_P2, fc5_P3, fc5_P4,fc5_P5
    ...
    def vec_rule_attention(self, cls_rule_vec, fc5_P0, fc5_P1, fc5_P2, fc5_P3, fc5_P4, fc5_P5,fc5_O, initializer, is_training,name):
        with tf.variable_scope(name) as scope:
            print('type:',type(fc5_P0.shape[0].value))
            fc5_P0 = tf.multiply(fc5_P0, tf.reshape((cls_rule_vec[:,:,0:1]+cls_rule_vec[:,:,3:4])/2,[-1,512]))
            fc5_P1 = tf.multiply(fc5_P1, tf.reshape((cls_rule_vec[:,:,1:2]+cls_rule_vec[:,:,2:3])/2,[-1,512]))
            fc5_P2 = tf.multiply(fc5_P2, tf.reshape(cls_rule_vec[:,:,4:5],[-1,512]))
            fc5_P3 = tf.multiply(fc5_P3, tf.reshape((cls_rule_vec[:,:,6:7]+cls_rule_vec[:,:,9:10])/2,[-1,512]))
    ...
    def P_classification(self, fc7_P, is_training, initializer, name):
        with tf.variable_scope(name) as scope:
            ...
            print ("clsprobp:",cls_score_P.get_shape())
            cls_prob_P  = tf.nn.sigmoid(cls_score_P, name='cls_prob_P') 
            print ("clsprobp:",cls_prob_P.get_shape())
    ...
    def A1_classification(self,cls_rule_vec, fc7_P0, fc7_P1, fc7_P2,fc7_P3,fc7_P4,fc7_P5,fc5_O, is_training, initializer, name):
        with tf.variable_scope(name) as scope:
            cls_score_P0 = slim.fully_connected(fc7_P0, self.num_classes, 
                                               weights_initializer=initializer,
                                               trainable=is_training,
                                               activation_fn=None, scope='cls_score_P0')
            cls_score_P1 = slim.fully_connected(fc7_P1, self.num_classes, 
                                               weights_initializer=initializer,
                                               trainable=is_training,
                                               activation_fn=None, scope='cls_score_P1')
            cls_score_P2 = slim.fully_connected(fc7_P2, self.num_classes, 
                                               weights_initializer=initializer,
                                               trainable=is_training,
                                               activation_fn=None, scope='cls_score_P2')
            cls_score_P3 = slim.fully_connected(fc7_P3, self.num_classes, 
                                               weights_initializer=initializer,
                                               trainable=is_training,
                                               activation_fn=None, scope='cls_score_P3')
            cls_score_P4 = slim.fully_connected(fc7_P4, self.num_classes, 
                                               weights_initializer=initializer,
                                               trainable=is_training,
                                               activation_fn=None, scope='cls_score_P4')
            cls_score_P5 = slim.fully_connected(fc7_P5, self.num_classes, 
                                               weights_initializer=initializer,
                                               trainable=is_training,
                                               activation_fn=None, scope='cls_score_P5')
            cls_score_PO = slim.fully_connected(fc5_O, self.num_classes, 
                                               weights_initializer=initializer,
                                               trainable=is_training,
                                               activation_fn=None, scope='cls_score_PO')
            cls_score_P0 = tf.multiply(cls_score_P0,(cls_rule_vec[:,:,0:1]+cls_rule_vec[:,:,3:4])/2)
            cls_score_P1 = tf.multiply(cls_score_P1,(cls_rule_vec[:,:,1:2]+cls_rule_vec[:,:,2:3])/2)
            cls_score_P2 = tf.multiply(cls_score_P2,cls_rule_vec[:,:,4:5])
            cls_score_P3 = tf.multiply(cls_score_P3,(cls_rule_vec[:,:,6:7]+cls_rule_vec[:,:,9:10])/2)
            cls_score_P4 = tf.multiply(cls_score_P4,(cls_rule_vec[:,:,7:8]+cls_rule_vec[:,:,8:9])/2) 
            cls_score_P5 = tf.multiply(cls_score_P5,(cls_rule_vec[:,:,5:6]))
            print('cls_P0:',cls_score_P0.get_shape())
            print('cls_score_P:',cls_score_P.get_shape())
            cls_score_P = tf.add(cls_score_P0,cls_score_P1)
            cls_prob_P  = tf.nn.sigmoid(cls_score_P, name='cls_prob_A') 
            tf.reshape(cls_prob_P, [1, self.num_classes]) 
            self.predictions["cls_score_A"] = cls_score_P
            self.predictions["cls_prob_A"]  = cls_prob_P
...
#   print ( "fc7p0:",fc7_P0.get_shape())
        fc7_P = tf.concat([fc7_P0, fc7_P1, fc7_P2, fc7_P3, fc7_P4, fc7_P5], axis=1)
#       print('fc7_P:',fc7_P.get_shape())
        self.vec_classification(fc7_P, is_training, initializer, 'vec_classification') 
        
        fc7_P_att = self.vec_attention(self.predictions['cls_prob_vec'],self.cls_rule_vec, fc7_P0, fc7_P1, fc7_P2, fc7_P3, fc7_P4, fc7_P5, fc5_O, initializer, is_training, name='vec_attention')
#       fc7_P_att = self.vec_rule_attention(self.cls_rule_vec, fc8_P0, fc8_P1, fc8_P2, fc8_P3, fc8_P4, fc8_P5, fc5_O,initializer, is_training,name = 'vec_rule_attention')
        fc7_L = self.language_head()
#       fc8_P = tf.concat([fc8_P0,fc8_P1,fc8_P2,fc8_P3,fc8_P4,fc8_P5],axis = 1)
        self.verb_classification(fc7_P, is_training, initializer, 'verb_classification') 

#       print('xl fc7_P: ', fc7_P.get_shape())
#       print('xl fc8_P: ', fc8_P.get_shape())
        self.P_classification(fc7_P, is_training, initializer, 'region_classification') 
#       print('xl fc7_P_att: ', fc7_P_att.get_shape())
#        self.A1_classification(self.cls_rule_vec,fc8_P0,fc8_P1,fc8_P2,fc8_P3,fc8_P4,fc8_P5,fc5_O,is_training,initializer,'region_classification')
    ...
    def train_step(self, sess, blobs, lr, train_op):
        feed_dict = {self.image: blobs['image'], 
                     ...
                     self.lr: lr, self.H_num: blobs['H_num'], self.cls_rule_vec: blobs['cls_rule_vec']} 
    ...
    def test_image_HO(self, sess, image, blobs):
        feed_dict = {self.image: image, 
                     self.H_boxes: blobs['H_boxes'], self.P_boxes: blobs['P_boxes'], self.R_boxes: blobs['R_boxes'], self.O_boxes: blobs['O_boxes'], 
                     self.H_num: blobs['H_num'], self.gt_object: blobs['gt_object'], self.cls_rule_vec: blobs['cls_rule_vec'] }
