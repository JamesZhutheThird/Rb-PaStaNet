...
def Generate_action(cls_rule,action_list):
...
def im_detect(sess, net, image_id, Test_RCNN, keys, scores_P, scores_A, scores_L, bboxes, hdet, odet):
    ...
    blobs        = {}
    rule_data = np.loadtxt('/disk1/AI004/PaSta_NEW/HAKE-Action/Weights/rule_power.txt',dtype=np.float64,delimiter=',')
    ...
    # for Human_out in Test_RCNN[image_id]:
        ...
        if Human_out[2] == 1:
            ...
            blobs['cls_rule_vec'] = [np.empty((0,512,10),dtype = np.float64)]
            ...
            for j in range(len(Test_RCNN[image_id])):
                if i != j: 
                    ...
                    action_list = []
                    class_id = Object[2] - 1
                    x,y = obj_range[class_id]
                    x -= 1
                    for hoi_id in range(x, y):
                        gt_bbox = pickle.load(open('/disk1/AI004/PaSta_NEW/HAKE-Action/-Results/gt_hoi_py2/hoi_%d.pkl' % hoi_id, 'rb'))
                        if image_id in gt_bbox:
                            action_list.append(hoi_id)
                    blobs['cls_rule_vec'].append(Generate_action(rule_data,action_list))
            ...
            if blobs['H_num'] > 0:
                blobs['cls_rule_vec'] = np.concatenate(blobs['cls_rule_vec'], axis=0)  
